...
# custom build for podmigration(SSU-DCN)
# https://github.com/SSU-DCN/podmigration-operator.git
SSU_DCN_POD_MIGRATION_PKG_DIR=$PKG_DIR/podmigration-operator
SSU_DCN_KUBEADM_BUILD_BIN=$SSU_DCN_POD_MIGRATION_PKG_DIR/custom-binaries/kubeadm
SSU_DCN_KUBELET_BUILD_BIN=$SSU_DCN_POD_MIGRATION_PKG_DIR/custom-binaries/kubelet
SSU_DCN_CONTAINERD_BUILD_BIN=$SSU_DCN_POD_MIGRATION_PKG_DIR/custom-binaries/containerd
...
if [[ ! -d $SSU_DCN_DEP_KUBERNETES_PKG_DIR ]]; then
    error "Not found $SSU_DCN_DEP_KUBERNETES_PKG_DIR directory. Try 'make build'"
fi
...
if [[ ! -d $SSU_DCN_POD_MIGRATION_PKG_DIR ]]; then
    error "Not found $SSU_DCN_POD_MIGRATION_PKG_DIR directory. Try 'make build'"
fi

if [[ ! -f $SSU_DCN_KUBEADM_BUILD_BIN ]]; then
    error "Not found $SSU_DCN_KUBEADM_BUILD_BIN file"
fi

if [[ ! -f $SSU_DCN_KUBELET_BUILD_BIN ]]; then
    error "Not found $SSU_DCN_KUBELET_BUILD_BIN file"
fi

if [[ ! -f $SSU_DCN_CONTAINERD_BUILD_BIN ]]; then
    error "Not found $SSU_DCN_CONTAINERD_BUILD_BIN file"
fi
