...
chmod +x $SSU_DCN_KUBEADM_BUILD_BIN
chmod +x $SSU_DCN_KUBELET_BUILD_BIN

cp -f $SSU_DCN_KUBEADM_BUILD_BIN $KUBEADM_BIN
cp -f $SSU_DCN_KUBELET_BUILD_BIN $KUBELET_BIN
...
# Install criu
cd $SSU_DCN_CRIU_PKG_DIR && make install && criu check
